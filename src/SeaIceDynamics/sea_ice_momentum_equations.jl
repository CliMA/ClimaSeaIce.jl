...
- `free_drift`: The free drift velocities used by free-drift sea ice dynamics. Default is `nothing`.
- `solver`: The momentum solver to be used.
- `minimum_concentration`: The minimum sea ice concentration above which the sea ice velocity is dynamically calculated; below this threshold velocity is set to zero. Default is `1e-3`.
- `minimum_mass`: The minimum sea ice mass per area above which the sea ice velocity is dynamically calculated; below this threshold velocity is set to zero. Default is `1.0 kg/m²`.
"""
