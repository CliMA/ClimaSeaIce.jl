...
@inline function advective_bgrid_tracer_flux_x(i, j, k, grid, advection, u, c)
    ũ  = ℑyᵃᶜᵃ(i, j, k, grid, u)
    cᴿ = _biased_interpolate_xᶠᵃᵃ(i, j, k, grid, advection, bias(ũ), c)
    return Axᶠᶜᶜ(i, j, k, grid) * ũ * cᴿ
end

@inline function advective_bgrid_tracer_flux_y(i, j, k, grid, advection, v, c)
...
@inline horizontal_div_Uc(i, j, k, grid, advection, U, c) = 
    1 / Vᶜᶜᶜ(i, j, k, grid) * (δxᶜᵃᵃ(i, j, k, grid, advective_bgrid_tracer_flux_x, advection, U.u, c) +
                               δyᵃᶜᵃ(i, j, k, grid, advective_bgrid_tracer_flux_y, advection, U.v, c))
                               
@inline horizontal_div_Uc(i, j, k, grid, advection::FluxFormAdvection, U, c) = 
    1 / Vᶜᶜᶜ(i, j, k, grid) * (δxᶜᵃᵃ(i, j, k, grid, advective_bgrid_tracer_flux_x, advection.x, U.u, c) +
                               δyᵃᶜᵃ(i, j, k, grid, advective_bgrid_tracer_flux_y, advection.y, U.v, c))
